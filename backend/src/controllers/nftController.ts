...
// Generate and Mint NFT Controller
export const generateAndMintNFT = async (req: Request, res: Response) => {
  ...
  if (
    ...
    return res.status(400).json({
      error: 'Invalid input types. "prompt" and "walletAddress" must be strings, and "length" must be a number.',
    });
  }

  try {
    // Generate unique output file name and path
    ...
    // Step 3: Mint NFT using Ethers.js
    ...
    return res.status(500).json({
      error: 'Audio generation, upload, or NFT minting failed',
